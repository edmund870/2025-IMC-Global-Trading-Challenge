...
class Parser:
    ...
    def convert_log(self):
        ...
        for i in range(len(arr_data)):
            arr_data[i] = {
                **arr_data[i],
                **self.process_log(json.loads(arr_data[i]["lambdaLog"])),
            }
        ...
        if self.flatten:
            ...
            for log in arr_data:
                for listing in log["listings"]:
                    ...
                    fdl["listing"] = listing
                    fdl["orderDepths"] = (
                        log["orderDepths"][listing]
                        if listing in log["orderDepths"]
                        else None
                    )
                    fdl["midPrice"] = (
                        log["midPrice"][listing] if listing in log["midPrice"] else None
                    )
                    fdl["orders"] = self.flatten_orders(log["orders"], listing)
                    fdl["ownTrades"] = self.flatten_trades(
                        log["ownTrades"], listing, log["timestamp"]
                    )
                    fdl["marketTrades"] = self.flatten_trades(
                        log["marketTrades"], listing, log["timestamp"]
                    )
                    fdl["position"] = (
                        log["position"][listing] if listing in log["position"] else None
                    )
    ...
    def get_pnl(
        ...
        return pd.read_csv(StringIO(filtered_content), sep=";", header=0)[
            ["timestamp", "product", "profit_and_loss"]
        ]
    ...
    def get_pd(self, flatten=False):
        if flatten:
            ...
            activity = pd.json_normalize(self.log)
            activity = pd.merge(
                activity,
                self.pnl,
                how="left",
                left_on=["timestamp", "listing"],
    ...
    def flatten_trades(self, trades, listing, time_now):
        flatten_trades = []
        for trade in trades:
            if trade[0] == listing:
                new_trade = {}
                ...
                new_trade["timestamp_now"] = time_now
