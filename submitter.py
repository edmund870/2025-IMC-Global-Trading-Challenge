...
def request_with_token(
    ...
    if response.status_code == 503:
        print(
            "The Prosperity website is currently in maintenance mode, please try again later"
        )
        sys.exit(1)
...
def download_logs(data: dict[str, Any], output_file: Path) -> None:
    ...
    url_response = request_with_token(
        "GET", f"{API_BASE_URL}/submission/logs/{data['id']}"
    )
...
def submit(
    algorithm_file: Path, output_file: Optional[Path], open_visualizer_on_success: bool
) -> None:
