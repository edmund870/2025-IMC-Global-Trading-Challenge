...
class fair_value:
    ...
    def gradient_descent(
        ...
        X2_diff,
        initial_params,
        learning_rate=np.array([1.468e-3, 1.875e-5, 1.55e-5]),
        max_iter=100,
        tolerance=1e-5,
    ...
    def KELP_FV(
        self,
        mid_cache: deque,
        most_micro_cache: deque,
        most_mid_cache: deque,
        forecast_cache: deque,
    ) -> tuple[int, int, float]:
        most_mid = self.order_book.most_mid_price()
        most_micro = self.order_book.most_micro_price()
        mid_price = self.order_book.mid_price()

        if len(most_micro_cache) > 1:
            calib_params = self.gradient_descent(
                mu=self.product_params["mu"],
                sigma2=self.product_params["sigma2"],
                y_diff=np.diff(mid_cache),
                X1_diff=np.diff(most_mid_cache),
                X2_diff=np.diff(most_micro_cache),
                initial_params=[-0.637, 0.464, -0.225],
            )
            ...
            theta_1 = self.product_params["MA1_coeff"]
            alpha_1 = self.product_params["most_mid_coeff"]
            alpha_2 = self.product_params["most_micro_coeff"]

        if len(most_micro_cache) >= 1:
            micro_delta = most_micro - most_micro_cache[-1]
            most_mid_delta = most_mid - most_mid_cache[-1]
        else:
            micro_delta = 0
            most_mid_delta = 0
        ...
        return mid_price, most_micro, most_mid, forecast
    ...
    def KELP_SQUID_INK_FV(self, most_mid_kelp, most_mid_squid, coin_forecast_cache):
        ...
        forecast = self.product_params["alpha"] @ (
            self.product_params["beta"] @ most_mid_price
        ) + self.product_params["gamma"] @ (most_mid_price - prev_forecast)

        squid_forecast = most_mid_squid + forecast[0]
        kelp_forecast = most_mid_kelp + forecast[1]
...
class trading_strategy:
    ...
    def market_make_SQUID(self, fv_orders) -> list[Order]:
        orders = []

        if self.max_buy > 0 and fv_orders[0].quantity < 0:
            highest_buy = self.order_book.high_volume_quotes(quote=quotes.bid)[0] + 1
            orders += self.rest_order(
                quote=quotes.bid, price=highest_buy, quantity=self.max_buy
            )

        if self.max_sell > 0 and fv_orders[0].quantity > 0:
            highest_sell = self.order_book.high_volume_quotes(quote=quotes.ask)[0] - 1
...
class Trader:
        ...
        if params is not None:
            ...
            self.params = {
                ...
                tradable_product.KELP: {
                    ...
                    "gamma": np.array(
                        [[-0.07401955, -0.05825519], [-0.00298338, -0.18217324]]
                    ),
                    "decay": 0.6,
                    "threshold": 0.8,
                },
                tradable_product.SQUID_INK: {
                    "alpha": np.array([[-2.91322007e-04], [3.69952085e-05]]),
                    "beta": np.array([[1.0, -0.97607176]]),
                    "gamma": np.array(
                        [[-0.10405293, -0.29092612], [-0.01459373, -0.4477722]]
                    ),
                },
            }
    ...
    most_micro_cache = copy.deepcopy(empty_dict_cache)
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == tradable_product.KELP:
                ...
                forecast = kelp_FV.KELP_SQUID_INK_FV(
            ...
            if product == tradable_product.SQUID_INK:
                ...
                forecast = squid_FV.KELP_SQUID_INK_FV(
                    most_mid_kelp=orderbook(
                        order_depth=state.order_depths[tradable_product.KELP]
                    ).mid_price(),
                    most_mid_squid=orderbook(
                ...
                MM_orders = []
                if len(fv_arb_buy_orders) > 0:
                    MM_orders = squid_strat.market_make_SQUID(
                        fv_orders=fv_arb_buy_orders
                    )
                if len(fv_arb_sell_orders) > 0:
                    MM_orders = squid_strat.market_make_SQUID(
                        fv_orders=fv_arb_sell_orders
                    )
