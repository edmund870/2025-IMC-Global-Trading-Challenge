...
class trading_strategy:
        ...
        if self.fair_value < 0:
            if self.max_buy > 0:
                for ask, ask_vol in self.order_book.asks:
                    ...
                    self.max_buy -= buy_amount
        if (
            self.fair_value >= self.product_params.get("exit_spread", 0)
            and self.position_pct > 0
        ):
            pos = abs(self.position)
            for bid, bid_vol in self.order_book.bids:
                bid_vol = abs(bid_vol)
                sell_amount = min(bid_vol, pos)
                orders += [Order(self.product, bid, -sell_amount)]
                pos -= bid_vol
                self.max_sell -= sell_amount
    ...
    def BUTTERFLY_ARB(self, leg):
        ...
        exit_spread = self.product_params.get("exit_spread", 0)
        if leg == "SHORT":
            if self.fair_value < 0:
                if self.max_sell > 0:
                    for bid, bid_vol in self.order_book.bids:
                        ...
                        orders += [Order(self.product, bid, -sell_amount)]
                        self.max_sell -= sell_amount
            if self.fair_value > exit_spread and self.position_pct < 0:
                pos = abs(self.position)
                for ask, ask_vol in self.order_book.asks:
                    ask_vol = abs(ask_vol)
                    buy_amount = min(ask_vol, pos)
                    orders += [Order(self.product, ask, buy_amount)]
                    pos -= ask_vol
                    self.max_buy -= buy_amount
        if leg == "LONG":
    ...
    def TRADE_VOL(self, iv_cache, mid_cache):
        orders = []
        if len(iv_cache) < 10:
            return orders
        ...
        mid_cache = np.array(mid_cache)
        mid_cache_return = mid_cache[1:] / mid_cache[:-1] - 1
        mid_cache_vol = mid_cache_return * np.sqrt(252e3)

        diff = iv_cache[1:] - mid_cache_vol
        test_stat = self.fair_value - mid_cache_vol[-1]

        threshold = self.product_params.get("z_threshold", 0)
        z = self.z_score(test_stat, diff)
    ...
    def HEDGE(self, to_hedge):
        ...
        logger.print(hedge_amt, self.position, to_hedge)

        if hedge_amt > 0 and self.max_buy > 0:
            for ask, ask_vol in self.order_book.asks:
                ask_vol = abs(ask_vol)
                buy_amount = min(ask_vol, self.max_buy, hedge_amt)
                orders += [Order(self.product, ask, buy_amount)]
                hedge_amt -= ask_vol
                self.max_buy -= buy_amount
                if hedge_amt <= 0:
        ...
        if params is not None:
            ...
            self.params = {
                ...
                tradable_product.CROISSANTS: {
                    "alpha": np.array([[-0.00077897], [0.00356119]]),
                    "beta": np.array([[1.0, 0.07278235]]),
                    "gamma": np.array(
                        [[-0.13665529, 0.00551131], [0.46266129, -0.0553495]]
                    ),
                ...
                tradable_product.JAMS: {
                    "alpha": np.array([[-0.00020995], [0.0021751]]),
        ...
        for product in state.order_depths:
            ...
            if product in options:
                ...
                if product in butterfly:
                    for i in range(1, 3):
                        if idx + i < len(options) and idx - i >= 0:
                            butterfly_spread = product_fv.BUTTERFLY(
                                lower=orderbook(
                                    order_depth=state.order_depths[options[idx - i]]
                                ).mid_price(),
                ...
                option_trades = option_strat.TRADE_VOL(
                    self.iv_cache[product],
                    self.mid_cache[tradable_product.VOLCANIC_ROCK],
                )
                ...
                result[product] = (
                    call_lb_orders + butterfly_orders + bcp_orders + option_trades
                )
