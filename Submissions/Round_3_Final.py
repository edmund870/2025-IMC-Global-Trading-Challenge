...
class fair_value:
    ...
    def solve_S(self, call_price, sigma, K, r, T, max_iter=1000, tol=1e-6):
        solution = self.order_book.mid_price()
        for _ in range(max_iter):
            f_val = self.black_scholes_call(solution, K, r, T, sigma) - call_price
            f_prime_val = self.call_delta(solution, K, r, T, sigma)
    ...
    def THEO_PRICE(self, S, K, r, TTE, mt_cache, coeffs):
        if coeffs is not None:
            ...
            price = self.black_scholes_call(S=S, K=K, r=r, sigma=iv_interpolated, T=TTE)
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for option in self.options:
            ...
            option_fv = fair_value(
                product=option,
                product_params=self.params[option],
                order_book=ob,
            )

            sigma = option_fv.solve_iv(
        ...
        for product in state.order_depths:
            ...
            if product in self.options:
                ...
                trade_optons_mispricing += option_strat.TRADE_OPTIONS()
                ...
                all_orders = {}
                for order in trade_optons_mispricing + call_lb_orders:
            ...
            if product == tradable_product.VOLCANIC_ROCK:
                ...
                if len(x) > 50:
                    strike_diff = [
                        abs(self.mt_cache[strike][-1]) for strike in self.options
                    ]
                    idx = strike_diff.index(min(strike_diff))
                    if min(strike_diff) < 0.005:
                        closest_strike = self.options[idx]
                        interpolated_iv = sum(
                            self.coeffs
                            * np.array(
                                [
                                    self.mt_cache[closest_strike][-1] ** 2,
                                    self.mt_cache[closest_strike][-1],
                                    1,
                                ]
                            )
                        )

                        implied_S = product_fv.solve_S(
                            call_price=orderbook(
                                order_depth=state.order_depths[closest_strike]
                            ).mid_price(),
                            sigma=interpolated_iv,
                            K=self.params[closest_strike]["strike"],
                            r=0,
                            T=t,
                        )
                        logger.print(implied_S, order_book.mid_price())
                        option_strat = trading_strategy(
                            product=product,
                            product_params=product_params,
                            position=curr_pos,
                    ...
                    rock_orders = option_strat.TRADE_VOLCANIC_ROCK()

                result[product] = rock_orders
