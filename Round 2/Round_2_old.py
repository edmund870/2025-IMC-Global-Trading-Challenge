...
empty_dict_cache = {
    "KELP": deque(maxlen=201),
    "RAINFOREST_RESIN": deque(maxlen=200),
    "SQUID_INK": deque(maxlen=200),
    "CROISSANTS": deque(maxlen=200),
    "JAMS": deque(maxlen=200),
    "PICNIC_BASKET1": deque(maxlen=200),
    "PICNIC_BASKET2": deque(maxlen=200),
    "DJEMBES": deque(maxlen=200),
}
...
class fair_value:
    ...
    def VECM(self, current_price, prev_forecast: deque):
        return self.product_params["alpha"] @ (
            self.product_params["beta"] @ current_price
        ) + self.product_params["gamma"] @ (current_price - prev_forecast)

    def KELP_SQUID_INK_FV(self, most_mid_kelp, most_mid_squid, coin_forecast_cache):
        most_mid_price = np.array([most_mid_squid, most_mid_kelp])

        if len(coin_forecast_cache) > 0:
            prev_forecast = coin_forecast_cache
        else:
            prev_forecast = most_mid_price

        delta = self.VECM(current_price=most_mid_price, prev_forecast=prev_forecast)

        squid_forecast = most_mid_squid + delta[0]
        kelp_forecast = most_mid_kelp + delta[1]

        forecast = np.array([squid_forecast, kelp_forecast])

        return delta, forecast

    def PICNIC_FV(self, mid_basket, mid_picnic, coin_forecast_cache):
        current_price = np.array([mid_picnic, mid_basket])

        if len(coin_forecast_cache) > 0:
            prev_forecast = coin_forecast_cache
        else:
            prev_forecast = current_price

        delta = self.VECM(current_price=current_price, prev_forecast=prev_forecast)

        mid_picnic = mid_picnic + delta[0]
        mid_basket = mid_basket + delta[1]

        forecast = np.array([mid_picnic, mid_picnic])
...
class trading_strategy:
    ...
    def trade_SQUID(self, forecast_cache, delta) -> list[Order]:
        orders = []

        forecast_cache = np.array(forecast_cache)
        z = (delta - forecast_cache.mean()) / forecast_cache.std()

        if z < -self.product_params.get("z_threshold", 0) and self.max_buy > 0:
            for i in range(0, self.order_book.depth(quote=quotes.ask)):
                if self.max_buy < 0:
                    break
                ...
                if self.max_buy - volume < 0:
                    volume = self.max_buy
                orders += [Order(self.product, price - 1, volume)]
                self.max_buy -= volume

        if z > self.product_params.get("z_threshold", 0) and self.max_sell > 0:
            for i in range(0, self.order_book.depth(quote=quotes.bid)):
                if self.max_sell < 0:
                    break
    ...
    def trade_PICNIC1(
        self, forecast_cache, delta, basket_pos, basket_order_depth
    ) -> list[Order]:
        orders = []
        ...
        if z < -self.product_params.get("z_threshold", 0) and self.max_buy > 0:
            for i in range(0, self.order_book.depth(quote=quotes.ask)):
                ...
                orders += [
                    Order(
                        self.product,
                        price - math.floor(self.order_book.spread() / 4),
                        volume,
                    )
                ]
                self.max_buy -= volume

        #         for cons in basket_pos.keys():
        #             cons_max_sell = position_limit[cons] + basket_pos[cons]
        #             to_sell = volume * self.product_params[cons]
        #             if cons_max_sell > 0:
        #                 for i in range(0, self.order_book.depth(quote=quotes.bid)):
        #                     if cons_max_sell < 0:
        #                         break
        #                     price, volume = self.order_book.get_level(quote=quotes.bid, level=i)
        #                     volume = abs(volume)
        #                     if cons_max_sell - volume < 0:
        #                         volume = cons_max_sell
        #                     orders += [Order(self.product, price - 1, -volume)]
...
class Trader:
        ...
        if params is not None:
            ...
            self.params = {
                ...
                tradable_product.PICNIC_BASKET1: {
                    ...
                    "alpha": np.array([[-8.56913229e-04], [-1.96619639e-05]]),
                    "beta": np.array([[1.0, 1.00079511]]),
                    "gamma": np.array(
                        [[-0.0654986, 0.07885795], [0.06257491, -0.13651877]]
                    ),
                    "z_threshold": 1,
                },
                tradable_product.PICNIC_BASKET2: {
                    ...
                    "alpha": np.array([[-5.46358375e-04], [7.20794033e-05]]),
                    "beta": np.array([[1.0, -1.00082926]]),
                    "gamma": np.array(
                        [[-0.05265516, 0.07138386], [0.08103845, -0.15665631]]
                    ),
                    "z_threshold": 1,
                },
    ...
    coin_forecast_cache_picnic1 = []
    coin_forecast_cache_picnic2 = []
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            basket_pos = {
                tradable_product.CROISSANTS: state.position.get(
                    tradable_product.CROISSANTS, 0
                ),
                tradable_product.JAMS: state.position.get(tradable_product.JAMS, 0),
                tradable_product.DJEMBES: state.position.get(
            ...
            if product == tradable_product.KELP:
                ...
                _, forecast = kelp_FV.KELP_SQUID_INK_FV(
                    most_mid_kelp=orderbook(
                        order_depth=state.order_depths[tradable_product.KELP]
                    ).mid_price(),
                    most_mid_squid=orderbook(
                        order_depth=state.order_depths[tradable_product.SQUID_INK]
                    ).mid_price(),
                    coin_forecast_cache=self.coin_forecast_cache_kelp,
                )
            ...
            if product == tradable_product.SQUID_INK:
                ...
                delta, forecast = squid_FV.KELP_SQUID_INK_FV(
                    most_mid_kelp=orderbook(
                        order_depth=state.order_depths[tradable_product.KELP]
                    ).mid_price(),
                    most_mid_squid=orderbook(
                    ...
                    coin_forecast_cache=self.coin_forecast_cache_squid,
                )
                ...
                squid_strat = trading_strategy(
                    ...
                    order_book=order_book,
                    fair_value=forecast[0],
                )

                fv_arb_buy_orders = squid_strat.fv_arb(quote=quotes.ask)
                fv_arb_sell_orders = squid_strat.fv_arb(quote=quotes.bid)

                squid_orders = squid_strat.trade_SQUID(
                    forecast_cache=self.coin_forecast_cache_ts[product],
                    delta=delta[0],
                )

                result[product] = fv_arb_buy_orders + fv_arb_sell_orders + squid_orders

                self.coin_forecast_cache_squid = forecast
            ...
            if product == tradable_product.PICNIC_BASKET1:
                cross = (
                    ...
                    * product_params[tradable_product.CROISSANTS]
                )

                jams = (
                    basket_order_depth[tradable_product.JAMS].mid_price()
                    * product_params[tradable_product.JAMS]
                )

                dj = (
                    basket_order_depth[tradable_product.DJEMBES].mid_price()
                    * product_params[tradable_product.DJEMBES]
                )

                basket_price = cross + jams + dj

                picnic_FV = fair_value(
                    product=product,
                    product_params=product_params,
                    order_book=order_book,
                )
                delta, forecast = picnic_FV.PICNIC_FV(
                    mid_basket=basket_price,
                    mid_picnic=order_book.mid_price(),
                    coin_forecast_cache=self.coin_forecast_cache_picnic1,
                )
                ...
                picnic1_orders = picnic1_strat.trade_PICNIC1(
                    forecast_cache=self.coin_forecast_cache_ts[product],
                    delta=delta[0],
                    basket_pos=basket_pos,
                    basket_order_depth=basket_order_depth,
                )

                result[product] = picnic1_orders

                self.coin_forecast_cache_picnic1 = forecast
            ...
            if product == tradable_product.PICNIC_BASKET2:
                ...
                self.coin_forecast_cache_picnic2 = forecast
                self.coin_forecast_cache_ts[product].append(delta[0])
